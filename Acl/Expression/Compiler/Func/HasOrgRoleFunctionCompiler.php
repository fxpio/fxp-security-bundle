 ...
 */
class HasOrgRoleFunctionCompiler implements FunctionCompilerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    public function compilePreconditions(ExpressionCompiler $compiler, FunctionExpression $function)
    {
        if (1 !== count($function->args)) {
            throw new JmsRuntimeException(sprintf('The hasOrgRole() function expects exactly one argument, but got "%s".', var_export($function->args, true)));
        }
    }
