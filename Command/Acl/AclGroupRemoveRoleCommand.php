 ...
 */
class AclGroupRemoveRoleCommand extends ContainerAwareCommand
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->setName('acl:group:remove:role')
        ->setDescription('Remove role in group')
        ->setDefinition(array(
                new InputArgument('group-name', InputArgument::REQUIRED, 'The group name'),
                new InputArgument('role-name', InputArgument::REQUIRED, 'The role name to dissociated in group'),
         ));
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        try {
            ...
            if (!in_array($role->getRole(), $group->getRoles())) {
                throw new \InvalidArgumentException("The role '$roleName' is no associated with group '$groupName'");
            }
            ...
            $output->writeln(array('', "The role <info>$roleName</info> was successfully dissociated with group <info>$groupName</info>"));
