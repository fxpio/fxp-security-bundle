 ...
 */
class AddChildCommand extends ContainerAwareCommand
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->setName('security:role:child:add')
        ->setDescription('Add child role')
        ->setDefinition(array(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($role->hasChild($childName)) {
            $output->writeln(sprintf('Role "%s" did already have "%s" child role.', $roleName, $childName));
        ...
        $output->writeln(sprintf('Child role "%s" has been added to role "%s".', $childName, $roleName));
    }
